...
class AlertPolicyConfig:
    """Governed, validation-tunable operating thresholds.

    Defaults are conservative working values, not fitted production cutoffs.
    A promoted model should replace them with frozen thresholds selected only
    from training/validation data and record the resulting policy version.
    """

    amber_hazard_threshold: float = 0.10
    red_hazard_threshold: float = 0.20
    mechanism_evidence_threshold: float = 70.0
    minimum_corroborating_mechanisms: int = 2
    minimum_mechanism_confidence: float = 0.50
    minimum_overall_confidence: float = 0.50
    minimum_persistent_periods: int = 2
    require_persistence_for_red: bool = True
    policy_version: str = "provisional-v1"

    def __post_init__(self) -> None:
        probabilities = {
            "amber_hazard_threshold": self.amber_hazard_threshold,
            "red_hazard_threshold": self.red_hazard_threshold,
            "minimum_mechanism_confidence": self.minimum_mechanism_confidence,
            "minimum_overall_confidence": self.minimum_overall_confidence,
        }
        for name, value in probabilities.items():
            if not 0 <= value <= 1:
                raise ValueError(f"{name} must be between 0 and 1")
        if self.red_hazard_threshold < self.amber_hazard_threshold:
            raise ValueError("red_hazard_threshold must be at least the Amber threshold")
        if not 0 <= self.mechanism_evidence_threshold <= 100:
            raise ValueError("mechanism_evidence_threshold must be between 0 and 100")
        if self.minimum_corroborating_mechanisms < 1:
            raise ValueError("minimum_corroborating_mechanisms must be at least 1")
        if self.minimum_persistent_periods < 1:
            raise ValueError("minimum_persistent_periods must be at least 1")


def _aligned_series(
    values: pd.Series | Sequence[float] | float,
    index: pd.Index,
    name: str,
) -> pd.Series:
    ...
    if isinstance(values, pd.Series):
        if len(values) != len(index):
            raise ValueError(f"{name} must have one value per evidence row")
        if values.index.equals(index):
            return values.rename(name)
        return pd.Series(values.to_numpy(), index=index, name=name)
    sequence = list(values)
    if len(sequence) != len(index):
        raise ValueError(f"{name} must have one value per evidence row")
    return pd.Series(sequence, index=index, name=name)


def apply_alert_policy(
    systemic_hazard: pd.Series | Sequence[float] | float,
    evidence: MechanismEvidenceResult,
    *,
    persistence_periods: pd.Series | Sequence[float] | float | None = None,
    config: AlertPolicyConfig | None = None,
) -> pd.DataFrame:
    """Translate systemic hazard into None/Amber/Red review states.

    Red requires a high probability, multiple independently eligible mechanism
    groups, adequate observed evidence, and (by default) persistence.  A high
    probability that fails corroboration or persistence remains Amber.  A row
    with inadequate evidence is explicitly labelled ``insufficient_evidence``
    instead of being forced into a risk class.
    """

    config = config or AlertPolicyConfig()
    summary = evidence.summary.reset_index(drop=True).copy()
    index = summary.index
    hazard = pd.to_numeric(
        _aligned_series(systemic_hazard, index, "systemic_hazard"),
        errors="coerce",
    )
    if persistence_periods is None:
        persistence = pd.Series(0.0, index=index, name="persistence_periods")
        persistence_supplied = False
    else:
        persistence = pd.to_numeric(
            _aligned_series(persistence_periods, index, "persistence_periods"),
            errors="coerce",
        ).fillna(0.0)
        persistence_supplied = True

    mechanism_table = evidence.mechanism_evidence
    output_rows: list[dict] = []
    for row_position in index:
        mechanisms = mechanism_table[
            mechanism_table["row_position"] == row_position
        ]
        corroborating = mechanisms[
            mechanisms["corroboration_eligible"].fillna(False)
            & (
                mechanisms["risk_evidence"]
                >= config.mechanism_evidence_threshold
            )
            & (
                mechanisms["evidence_confidence"]
                >= config.minimum_mechanism_confidence
            )
        ]
        corroborating_keys = corroborating["mechanism"].astype(str).tolist()
        probability = (
            float(hazard.iloc[row_position])
            if pd.notna(hazard.iloc[row_position])
            else np.nan
        )
        overall_confidence = summary.loc[
            row_position, "overall_evidence_confidence"
        ]
        confidence_ok = (
            pd.notna(overall_confidence)
            and overall_confidence >= config.minimum_overall_confidence
        )
        persistence_ok = (
            (not config.require_persistence_for_red)
            or (
                persistence_supplied
                and persistence.iloc[row_position] >= config.minimum_persistent_periods
            )
        )
        red_probability = (
            np.isfinite(probability)
            and probability >= config.red_hazard_threshold
        )
        amber_probability = (
            np.isfinite(probability)
            and probability >= config.amber_hazard_threshold
        )
        corroboration_ok = (
            len(corroborating_keys) >= config.minimum_corroborating_mechanisms
        )

        if not np.isfinite(probability) or not confidence_ok:
            alert = "insufficient_evidence"
        elif red_probability and corroboration_ok and persistence_ok:
            alert = "red"
        elif amber_probability:
            alert = "amber"
        else:
            alert = "none"

        blockers: list[str] = []
        if red_probability and alert != "red":
            if not confidence_ok:
                blockers.append("insufficient_evidence")
            if not corroboration_ok:
                blockers.append("insufficient_corroboration")
            if not persistence_ok:
                blockers.append("persistence_unconfirmed")

        output = summary.loc[row_position].to_dict()
        output.update(
            {
                "systemic_hazard": probability,
                "alert_level": alert,
                "corroborating_mechanism_count": len(corroborating_keys),
                "corroborating_mechanisms": tuple(corroborating_keys),
                "persistence_periods": float(persistence.iloc[row_position]),
                "persistence_confirmed": bool(persistence_ok),
                "red_blockers": tuple(blockers),
